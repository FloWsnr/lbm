...
// They are described in Palabos docs (ch 16), and they also call functionals "data processors". But they confuse me a lot!
...
// A function for future development of a scheme to adjust Ca to be between a certain threshold
//T AdjustForceForCapillaryNumber(T Ca_high, T Ca_low, ) {
//
//  T NewForce;
//
//  T Ca_f1 = computeCapillaryNumber_f1(lattice_fluid1, nu_f1);
//  T Ca_f2 = computeCapillaryNumber_f2(lattice_fluid2, nu_f2);
//
//  // NewForce = some relationship to keep Ca within a certain range [Ca_high, Ca_low]...
//
//  setExternalVector(lattice_fluid1, lattice_fluid1.getBoundingBox(),
//    DESCRIPTOR < T > ::ExternalField::forceBeginsAt, Array < T, 3 > (NewForce, 0., 0.));
//  setExternalVector(lattice_fluid2, lattice_fluid2.getBoundingBox(),
//    DESCRIPTOR < T > ::ExternalField::forceBeginsAt, Array < T, 3 > (NewForce, 0., 0.));
//
//  return NewForce
...
int main(int argc, char * argv[]) {
  ...
  plint max_iter; // max total iterations
  plint it_max_per_p;
  plint it_conv;
  // plint it_info ;
  plint it_vtk;
  ...
  if (load_state == true) {
    ...
    pcout << "Loading restart files..." << endl;
    try {
  ...
  for (plint runs = current_run_num; runs < runnum; ++runs) {
    ...
    while (checkconv == 0) { // Main loop over time iterations.
      ...
      if (total_iter >= max_iter) {
        pcout << "Reached maximum simulation iterations of " << max_iter << ". Aborting simulation." << endl;

        // Save restart files before exiting
        if (save_sim == true) {
          pcout << "Saving final restart files before aborting." << endl;
          saveBinaryBlock(lattice_fluid1, Lattice1);
          saveBinaryBlock(lattice_fluid2, Lattice2);

          string run_name = outDir + "/run_num.dat";
          plb_ofstream ofile_final_run(run_name.c_str());
          ofile_final_run << runs << endl;
          ofile_final_run.close();

          string iter_name = outDir + "/iter_num.dat";
          plb_ofstream ofile_final_iter(iter_name.c_str());
          ofile_final_iter << iT << endl;
      ...
      if (iT >= it_max_per_p) {
        pcout << "Reached maximum iterations for this pressure step: " << it_max_per_p
              << ". Moving to the next pressure step." << endl;
    ...
    //        pcout << "-----------------" << std::endl;
        ...
        pcout << "Ca fluid 1 = " << Ca_1 << std::endl;
        pcout << "Ca fluid 2 = " << Ca_2 << std::endl;
        pcout << "-----------------" << std::endl;
