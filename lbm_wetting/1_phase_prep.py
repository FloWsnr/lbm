"""
Prepares the 1-phase simulation.
...
from lbm_wetting.utils.palabos_file_1phase import PalabosInputFile1Phase
...
def prep_1_phase_sim(config: dict):
    """
    Prepare the 1-phase simulation.
    """
    ...
    palabos_file = PalabosInputFile1Phase(config)
...
def _check_prep_already_exists(sim_dir: Path, sim_name: str) -> bool:
    ...
    """
    return (sim_dir / sim_name / "input" / "1_phase_sim_input.xml").exists()
...
print("Starting preparation of 1-phase simulation...")
config = parse_input_file(config_path)
config = update_config(config, sim_dir, sim_name)
prep_1_phase_sim(config)
print("Done preparing 1-phase simulation!")
print("----------------------------------")
